...
this.btnList = {
    ...
    "horizontalrule": ['InsertHorizontalRule', '插入水平线',    'editor_action(this.id)',  'ed_hr.gif'],
    "createlink":     ['CreateLink',           '插入超连接',    'editor_action(this.id)',  'ed_link.gif'],
...
//    "htmlmode":       ['HtmlMode',             'View HTML Source',   'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          '扩大编辑器',     'editor_action(this.id)',  'fullscreen_maximize.gif'],
...
//      for (var thisName in document.all[objname].config) {
//	alert((document.all[objname].config)[thisName]);
//    }
  ...
  if (!config.width || config.width == "auto") {
    ...
    else if (obj.cols)        { config.width = (obj.cols * 8) + 22; }  // col width + toolbar(22)
...
function editor_action(button_id) {
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
   ...
   showModalDialog(_editor_url + "popups/insert_image1.htm", editdoc, "resizable: no; dialogWidth:680px; dialogHeight:440px; scroll: no; status: no;");
//	open("popups/insert_image.jsp");
    }
...
/*  for(var i=0;i<editdoc.body.getElementsByTagName('IMG').length&&editdoc.body.getElementsByTagName('IMG').length>0;i++)
  	images += ","+editdoc.body.getElementsByTagName('IMG')[i].src ;
	alert(images);
	document.all["imagesLink"].value = images;

	alert(editdoc.body.getElementsByTagName('IMG').length)//add by CoLin
*/
...
//      + ' oncontextmenu="parent.editor_cMenu_generate(window,\'' +objname+ '\');"'
  ...
  //else
  	//editor_updateOutput(objname);//add by chl
